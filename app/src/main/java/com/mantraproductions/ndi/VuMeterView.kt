...
/**
 * Hairline vertical audio meter, sitting down the left edge of the image.
 *
 * Deliberately thin: on a camera the picture is what you're looking at, and a
 * meter only needs to answer "is there audio and is it clipping". Scale is dB
 * rather than linear, because linear spends most of its length on levels you
 * never use.
 *
 * Peak hold falls back slowly so a transient clip stays visible long enough to
 * notice, which is the whole reason you glance at a meter mid-take.
 */
class VuMeterView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val top = h * (1f - level)
        // Green up to around -18dB equivalent, amber approaching the top, red at the very end.
